...
const WeatherInfo = ({ temperature, refetch }) => {
  const API_KEY_DAILY = `1fa9ff4126d95b8db54f3897a208e91c`;
  refetch();
  const lon = temperature.coord.lon;
  const lat = temperature.coord.lat;

  const url = {`https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&exclude=current,minutely,hourly,alerts&appid=${key}units=metric`}
